...
def display_text_art(file_name):
    try:
        ...
        print("Text art dosyası bulunamadı.")
...
def display_social_media(file_name):
    try:
        ...
        print("Sosyal medya dosyası bulunamadı.")
...
def download_igtv_and_post(username, log_file, L):
    try:
        ...
        with open(log_file, 'a', encoding='utf-8') as f:
            ...
            for story in profile.get_posts():
                print(f" - {story.url}")
                f.write(f" - {story.url}\n")
...
def main():
    ...
    #wexinsta adlı text art ım 
    display_text_art("textart.txt")
    
    
    #sosyal medyaları göster
    display_social_media("sosyal_medya.txt")

    while True:
        print("\n" + Colors.RED + "Seçenekler:" + Colors.END)
        ...
        print("4: IGTV Videoları ve Postları göster")
        ...
        if secim == "1":
            ...
            print("Beğenilen Gönderileri Göster seçeneği henüz eklenmemiştir.")
        elif secim == "4":
            download_igtv_and_post(kullanici_adi, log_file, L)
        elif secim == "5":
            download_profile_media(kullanici_adi, log_file, L)
        elif secim == "6":
            print("Video İndirme seçeneği henüz eklenmemiştir.")
        elif secim == "7":
